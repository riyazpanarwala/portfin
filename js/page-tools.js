...
function initUploadListeners() {
  const dzMF = document.getElementById("drop-zone-mf"),
    fiMF = document.getElementById("file-input-mf");
  dzMF.addEventListener("click", () => fiMF.click());
  dzMF.addEventListener("dragover", (e) => {
    e.preventDefault();
    dzMF.style.borderColor = "var(--gold)";
    dzMF.style.color = "var(--gold)";
  });
  dzMF.addEventListener("dragleave", () => {
    dzMF.style.borderColor = "";
    dzMF.style.color = "";
  });
  dzMF.addEventListener("drop", (e) => {
    e.preventDefault();
    dzMF.style.borderColor = "";
    handleExcel(e.dataTransfer.files[0], "mf");
  });
  fiMF.addEventListener("change", (e) => handleExcel(e.target.files[0], "mf"));

  const dzST = document.getElementById("drop-zone-st"),
    fiST = document.getElementById("file-input-st");
  dzST.addEventListener("click", () => fiST.click());
  dzST.addEventListener("dragover", (e) => {
    e.preventDefault();
    dzST.style.borderColor = "var(--gold)";
    dzST.style.color = "var(--gold)";
  });
  dzST.addEventListener("dragleave", () => {
    dzST.style.borderColor = "";
    dzST.style.color = "";
  });
  dzST.addEventListener("drop", (e) => {
    e.preventDefault();
    dzST.style.borderColor = "";
    handleExcel(e.dataTransfer.files[0], "st");
  });
...
function computeCAGR(invested, current, dates) {
  ...
  if (yrs < 0.5) return 0;
  return parseFloat(
    ((Math.pow(current / invested, 1 / yrs) - 1) * 100).toFixed(1),
  );
}
...
function parseMFRows(rows, dz, statusEl, fname) {
  ...
  if (!data.length) {
    statusEl.textContent = "✗ No MF data found — check column headers";
    dz.style.borderColor = "var(--red)";
    dz.style.color = "var(--red)";
    return;
  }
  ...
  if (!cScheme) {
    statusEl.textContent = "✗ Could not find Scheme column";
    dz.style.borderColor = "var(--red)";
    dz.style.color = "var(--red)";
    return;
  }
  ...
  if (!funds.length) {
    statusEl.textContent = "✗ No valid fund rows found";
    dz.style.borderColor = "var(--red)";
    dz.style.color = "var(--red)";
    return;
  }
...
function parseSTRows(rows, dz, statusEl, fname) {
  ...
  if (!data.length) {
    statusEl.textContent = "✗ No stock data found — check column headers";
    dz.style.borderColor = "var(--red)";
    dz.style.color = "var(--red)";
    return;
  }
  ...
  if (!cStock) {
    statusEl.textContent = "✗ Could not find Stock column";
    dz.style.borderColor = "var(--red)";
    dz.style.color = "var(--red)";
    return;
  }
  ...
  if (!stocks.length) {
    statusEl.textContent = "✗ No valid stock rows found";
    dz.style.borderColor = "var(--red)";
    dz.style.color = "var(--red)";
    return;
  }
  ...
  if (uniqUnclassified.length)
    stMsg += ` · ⚠ ${uniqUnclassified.length} stock(s) auto-mapped to "Other" sector (${uniqUnclassified.slice(0, 3).join(", ")}${uniqUnclassified.length > 3 ? "…" : ""})`;
...
function tryApplyData() {
  ...
  if (hasMF && hasST) {
    const funds = pendingMF.funds,
      stocks = pendingST.stocks;
...
function computeRebalance(targetMFPct, targetLCPct, targetETFPct) {
  ...
  if (kpiEl) {
    kpiEl.innerHTML = [
      ...
      .map(
        (c) =>
          `<div class="kpi-card" style="--accent:${c.a}"><div class="kpi-label">${c.l}</div><div class="kpi-value" style="font-size:18px">${c.v}</div><div class="kpi-sub">${c.s}</div></div>`,
      )
      .join("");
  }
...
function renderWaterfall() {
  ...
  document.getElementById("wf-kpis").innerHTML = [
    ...
    .map(
      (c) =>
        `<div class="kpi-card" style="--accent:${c.a}"><div class="kpi-label">${c.l}</div><div class="kpi-value" style="font-size:19px">${c.v}</div><div class="kpi-sub">${c.s}</div></div>`,
    )
    .join("");
  ...
  document.getElementById("wf-pills").innerHTML = pillsData
    .map(
      (p) => `
  ...
  const barGap = 18,
    barW = Math.floor((chartW - barGap * (n - 1)) / n);
  ...
  segments.forEach((seg, i) => {
    ...
    bars += `<rect ${glow} class="wf-bar-base" data-idx="${i}" x="${x}" y="${yTop.toFixed(1)}" width="${barW}" height="${bH.toFixed(1)}" fill="${seg.color}" opacity="${opacity}" rx="3" onmouseenter="wfShowTip(event,${i})" onmouseleave="wfHideTip()"/>`;
    const labelY = yTop - 6;
  ...
  const svgHTML = `<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;display:block">
    ${gridLines}${zeroLine}${connectors}${bars}${topLabels}${botLabels}
        ...
        const amtCls =
          seg.type === "gain"
            ...
            : seg.type === "loss"
              ? "td-dn"
              : "td-gold";
        return `<div class="wf-bk-row"><div class="wf-bk-dot" style="background:${seg.color}"></div><div class="wf-bk-name">${seg.label}</div><div class="wf-bk-amt ${amtCls}">${sign}${fmtL(Math.abs(seg.value))}</div><div class="wf-bk-pct">${pct}%</div></div>`;
      })
      .join("") +
    `</div>`;
  ...
  if (+mfRetPct > 0)
    insights.push({
      icon: "◎",
      text: `Mutual Funds returned <b>${pSign(+mfRetPct)}${mfRetPct}%</b> on your invested capital of ${fmtL(mfInvested)}.`,
    });
  if (stInvested)
    insights.push({
      icon: "◐",
      text: `Equity stocks returned <b>${pSign(+stRetPct)}${stRetPct}%</b> on your invested capital of ${fmtL(stInvested)}.`,
    });
  if (lossTotal > 0)
    insights.push({
      icon: "⚠",
...
function wfShowTip(e, idx) {
  ...
  document.getElementById("wf-tt-amt").textContent =
    (seg.value < 0 ? "−" : "") + fmtL(Math.abs(seg.value));
  document.getElementById("wf-tt-pct").textContent = total
  ...
  if (seg.subKey === "Return %") {
    ...
    document.getElementById("wf-tt-sub-v").textContent =
      inv > 0 ? fmtP((seg.value / inv) * 100) : "—";
  } else if (seg.subKey === "Total gain") {
    document.getElementById("wf-tt-sub-v").textContent = fmtL(
...
function renderSignal() {
  ...
  if (sipDay !== null) {
    if (daysToSIP <= 0) {
      ...
      signals.push({
        ...
        body: `Your SIP is due on the ${sipDay}${ordinal(sipDay)}. Keep funds ready in your bank account.`,
      ...
      signals.push({
        ...
        tag: "good",
        title: "SIP on track",
        body: `Next SIP due in ${Math.round(daysToSIP)} days on the ${sipDay}${ordinal(sipDay)}. No action needed this week.`,
  ...
  if (month1 >= 1 && month1 <= 3) {
    ...
    const totalHarvestable = stcgLosses.reduce(
      (a, s) => a + Math.abs(s.Gain || 0),
      0,
    );
    if (totalHarvestable > 0) {
      signals.push({
        ...
        body: `FY ends in ${daysToFY} days. You have STCG losses that can offset gains before March 31.`,
      ...
      signals.push({
        ...
        body: `March 31 deadline approaching. Review your LTCG gains — ₹1.25L is tax-free.`,
  ...
  if (deepLosers.length > 0) {
    ...
    signals.push({
      ...
      body: `${worst.name} is your worst performer at ${fmtP(worst.RetPct)}. Deep losses rarely recover without a clear catalyst.`,
  ...
  if (mfLosers.length > 0) {
    ...
    signals.push({
      ...
      body: `${worstMF.name} is your worst MF at ${fmtP(worstMF.RetPct)}.`,
  ...
  if (allLotDates.length > 0) {
    ...
    if (staleDays > 60) {
      signals.push({
        ...
        body: `Last investment was ${staleDays} days ago on ${fmtDate(latestInv)}.`,
  ...
  if (avgCandidates.length > 0) {
    signals.push({
      ...
      body: `${avgCandidates[0].name}${avgCandidates.length > 1 ? " and others" : ""} are slightly underwater but have positive long-term CAGR.`,
      metric: `${fmtP(avgCandidates[0].RetPct)} on ${avgCandidates[0].name.split(" ")[0]}`,
      metricClass: "info",
    });
  }
  ...
  if (stars.length > 0) {
    ...
    signals.push({
      ...
      body: `${best.name} is your star at ${fmtP(best.RetPct)}.`,
  ...
  document.getElementById("pas-badge").className =
    `pas-score-badge ${scoreClass}`;
  document.getElementById("pas-score-num").className =
    `pas-score-num ${scoreClass}`;
  ...
  document.getElementById("pas-mood-strip").innerHTML = moodItems
    .map(
      (m) =>
        `<div class="pas-mood-item"><div class="pas-mood-icon">${m.icon}</div><div><div class="pas-mood-label">${m.label}</div><div class="pas-mood-val">${m.val}</div></div></div>`,
    )
    .join("");
  ...
  for (let i = 0; i < 7; i++) {
    ...
    calHTML += `<div class="${cls2}"><div class="pas-cal-day-num" style="color:${isToday ? "var(--gold)" : isSIPDay ? "var(--blue)" : isFYEnd ? "var(--red)" : "var(--text)"}">${dd}</div><div class="pas-cal-day-label">${dayNote}</div><div class="pas-cal-day-dot" style="background:${dotColor}"></div></div>`;
  }
  ...
  document.getElementById("pas-action-grid").innerHTML = urgentFirst
    .map(
      (s) => `
    <div class="pas-action-card ${s.type}">
      <div class="pas-card-header"><span class="pas-card-icon">${s.icon}</span><span class="pas-card-tag ${s.tag}">${s.tag.toUpperCase()}</span></div>
  ...
  let checked = {};
  try {
    checked = JSON.parse(localStorage.getItem(weekKey) || "{}");
  } catch (e) {}
  const checklist = [
    {
      id: "sip",
  ...
  document.getElementById("pas-checklist").innerHTML = checklist
    .map((item) => {
      ...
      <div class="pas-check-text"><div class="pas-check-title">${item.title}</div><div class="pas-check-desc">${item.desc}</div></div>
    </div>`;
    })
    .join("");
}
