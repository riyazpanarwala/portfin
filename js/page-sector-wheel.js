...
function renderSectorWheel() {
  ...
  function getSignal(pct) {
    if (pct > equalWeight * 1.5)
      return { label: "OVERWEIGHT", color: "#f85149", arrow: "▲▲" };
    if (pct > equalWeight * 1.15)
      return { label: "SLIGHT OW", color: "#e3b341", arrow: "▲" };
    if (pct < equalWeight * 0.5)
      return { label: "UNDERWEIGHT", color: "#58a6ff", arrow: "▼▼" };
    if (pct < equalWeight * 0.85)
      return { label: "SLIGHT UW", color: "#a371f7", arrow: "▼" };
  ...
  // ── 5. Populate KPIs ──────────────────────────────────────
  const overweight = sectorData.filter(
    (s) => s.signal.label.includes("OW") || s.signal.label === "OVERWEIGHT",
  );
  const underweight = sectorData.filter(
    (s) => s.signal.label.includes("UW") || s.signal.label === "UNDERWEIGHT",
  );
  const topSec = sectorData[0];
  const mostConc = sectorData.reduce(
    (a, s) => (s.pct > a.pct ? s : a),
    sectorData[0],
  );
  ...
  document.getElementById("sw-kpis").innerHTML = [
    {
      l: "Sectors tracked",
      v: sectorData.length,
      s: "Active in your portfolio",
      a: "#58a6ff",
    },
    {
      l: "Largest exposure",
      v: topSec ? topSec.label : "—",
      s: topSec ? topSec.pct.toFixed(1) + "% of portfolio" : "—",
      a: topSec?.color || "#d4a843",
    },
    {
      l: "Overweight sectors",
      v: overweight.length || "—",
      s: overweight.length
        ? overweight.map((s) => s.label).join(", ")
        : "None — balanced",
      a: "#f85149",
    },
    {
      l: "Underweight",
      v: underweight.length || "—",
      s: underweight.length
        ? underweight.map((s) => s.label).join(", ")
        : "None",
      a: "#58a6ff",
    },
    {
      l: "Equal weight ref",
      v: equalWeight.toFixed(1) + "%",
      s: "Per sector, if equally split",
      a: "#7d8590",
    },
    {
      l: "MF implied",
      v: fmtL(Math.round(totalMFInv)),
      s: "Capital in funds",
      a: "#a371f7",
    },
  ]
  ...
  document.getElementById("sw-detail-rows").innerHTML = sectorData
    .map((s) => {
      const mfPct =
        totalMFInv > 0 ? ((s.mfV / totalMFInv) * 100).toFixed(1) : 0;
      const stPct =
        totalSTInv > 0 ? ((s.stV / totalSTInv) * 100).toFixed(1) : 0;
      const barW = Math.round((s.pct / maxPct) * 100);
      const mfBarW = Math.round(
        (s.mfV / (mfExp[s.key] > 0 ? grandTotal : 1)) * 100,
      );
      const stBarW = Math.round(
        (s.stV / (stExp[s.key] > 0 ? grandTotal : 1)) * 100,
      );
...
function swUnhighlight() {
  document
    .querySelectorAll(".sw-segment")
    .forEach((s) => (s.style.opacity = "0.85"));
  const tt = document.getElementById("sw-tooltip");
