...
function renderSnapshots() {
  ...
  const kpiData = [
    { l:'Portfolio value', v: fmtL(latest.totalValue),    s: wowTag(valDelta),      a:'#d4a843' },
    { l:'Total invested',  v: fmtL(latest.totalInvested), s: wowTag(invDelta),      a:'#58a6ff' },
    { l:'Total gain',      v: fmtL(latest.totalGain),     s: wowTag(gainDelta),     a: latest.totalGain >= 0 ? '#3fb950' : '#f85149' },
    { l:'MF CAGR',         v: fmtP(latest.mfCAGR),        s: wowTagPct(cagrDelta),  a:'#a371f7' },
    { l:'Snapshots saved', v: snapshots.length,            s: `Since ${sorted[0].label || sorted[0].shortLabel}`, a:'#7d8590' },
    { l:'Last updated',    v: latest.shortLabel || latest.label, s: new Date(latest.savedAt).toLocaleDateString('en-IN',{day:'2-digit',month:'short'}), a:'#f0c060' },
  ];

  const kpiHTML = kpiData
    .map(c => `<div class="kpi-card" style="--accent:${c.a}"><div class="kpi-label">${c.l}</div><div class="kpi-value">${c.v}</div><div class="kpi-sub">${c.s}</div></div>`)
    .join('');
  ...
  const tableRows = [...sorted].reverse().map((snap, i) => {
    ...
    const rDelta   = prevSnap && prevSnap.totalInvested
      ? ((snap.totalGain / snap.totalInvested) - (prevSnap.totalGain / prevSnap.totalInvested)) * 100
      : null;
    const isLatest = (snap.weekKey || snap.monthKey) === (latest.weekKey || latest.monthKey);
  ...
  scheduleChart('snap-chart-value', 60, (el) => {
    return new Chart(el, {
      ...
      options: {
        ...
        scales:{
          x:{ticks:{font:{size:9},color:'#7d8590',maxRotation:60,autoSkip:true,maxTicksLimit:16},grid:{color:'#21262d'}},
...
function exportSnapshotsCSV() {
  ...
  const headers = ['Week','Week Key','Total Value','Total Invested','Total Gain','Return %','MF CAGR %','MF Invested','Stock Invested','Funds','Stocks','Saved At'];
