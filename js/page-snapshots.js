...
function renderSnapshots() {
  ...
  // ── KPI strip ────────────────────────────────────────────────
  const momTag = (d) => {
    if (d === null) return '<span style="font-size:10px;color:var(--muted)">No prev</span>';
    const sign = d >= 0 ? '+' : '';
    const color = d >= 0 ? 'var(--green)' : 'var(--red)';
    return `<span style="font-size:10px;color:${color}">${sign}${fmtL(d)} MoM</span>`;
  };
  const momTagPct = (d) => {
    if (d === null) return '';
    const sign = d >= 0 ? '+' : '';
    const color = d >= 0 ? 'var(--green)' : 'var(--red)';
    return `<span style="font-size:10px;color:${color}">${sign}${d.toFixed(2)}pp MoM</span>`;
  };

  document.getElementById('snap-kpis').innerHTML = [
    {l:'Portfolio value',  v: fmtL(latest.totalValue),    s: momTag(valDelta),      a:'#d4a843'},
    {l:'Total invested',   v: fmtL(latest.totalInvested), s: momTag(invDelta),      a:'#58a6ff'},
    {l:'Total gain',       v: fmtL(latest.totalGain),     s: momTag(gainDelta),     a: latest.totalGain>=0?'#3fb950':'#f85149'},
    {l:'MF CAGR',          v: fmtP(latest.mfCAGR),        s: momTagPct(cagrDelta),  a:'#a371f7'},
    {l:'Snapshots saved',  v: snapshots.length,            s: `Since ${sorted[0].label}`, a:'#7d8590'},
    {l:'Last updated',     v: latest.label,                s: new Date(latest.savedAt).toLocaleDateString('en-IN',{day:'2-digit',month:'short'}), a:'#f0c060'},
  ...
  scheduleChart('snap-chart-value', 60, (el) => {
    return new Chart(el, {
      ...
      data: {
        labels,
        datasets: [
          {label:'Portfolio Value', data:sorted.map(s=>s.totalValue), borderColor:'#d4a843', backgroundColor:'rgba(212,168,67,.08)', borderWidth:2, pointRadius:3, pointHoverRadius:5, fill:true, tension:0.3},
          {label:'Total Invested',  data:sorted.map(s=>s.totalInvested), borderColor:'#58a6ff', backgroundColor:'transparent', borderWidth:1.5, pointRadius:2, borderDash:[4,3], tension:0.3},
        ]
      },
      options: {
  ...
  scheduleChart('snap-chart-cagr', 60, (el) => {
    return new Chart(el, {
      ...
      data: {
        labels,
        datasets: [
          {label:'MF CAGR %', data:sorted.map(s=>s.mfCAGR), borderColor:'#a371f7', backgroundColor:'rgba(163,113,247,.08)', borderWidth:2, pointRadius:3, pointHoverRadius:5, fill:true, tension:0.3},
        ]
      },
      options: {
        responsive:true, maintainAspectRatio:false,
        plugins:{
          ...
          tooltip:{callbacks:{label:ctx=>ctx.dataset.label+': '+fmtP(ctx.raw)},backgroundColor:'#1c2330',titleColor:'#e6edf3',bodyColor:'#7d8590',borderColor:'#30363d',borderWidth:1}
        },
        scales:{
          x:{ticks:{font:{size:9},color:'#7d8590',maxRotation:45},grid:{color:'#21262d'}},
          y:{ticks:{font:{size:9},color:'#7d8590',callback:v=>v.toFixed(1)+'%'},grid:{color:'#21262d'}}
        }
      }
    });
  });
  ...
  const tableRows = [...sorted].reverse().map((snap, i) => {
    const prevSnap = [...sorted].reverse()[i + 1];
    const vDelta = prevSnap ? snap.totalValue - prevSnap.totalValue : null;
    const rDelta = prevSnap && prevSnap.totalInvested ? ((snap.totalGain / snap.totalInvested) - (prevSnap.totalGain / prevSnap.totalInvested)) * 100 : null;
    const isLatest = snap.monthKey === latest.monthKey;
    return `<tr style="${isLatest?'background:var(--bg3)':''}">
      <td style="font-weight:${isLatest?'600':'400'}">${snap.label} ${isLatest?'<span style="font-size:9px;background:var(--gold);color:#0d1117;padding:1px 5px;border-radius:3px;margin-left:4px">LATEST</span>':''}</td>
      ...
      <td style="color:var(--a371f7)">${fmtP(snap.mfCAGR)}</td>
      <td>${vDelta!==null?`<span style="color:${vDelta>=0?'var(--green)':'var(--red)'};font-size:11px">${vDelta>=0?'+':''}${fmtL(vDelta)}</span>`:'<span style="color:var(--muted)">—</span>'}</td>
      <td class="td-muted">${snap.fundCount||'—'} / ${snap.stockCount||'—'}</td>
    </tr>`;
  }).join('');

  container.innerHTML = `
    <div class="kpi-grid" id="snap-kpis" style="margin-bottom:20px"></div>
    ...
    <div class="tbl-wrap">
      <table>
        <thead>
          <tr>
            <th>Month</th><th>Portfolio value</th><th>Invested</th>
            <th>Total gain</th><th>Return %</th><th>MF CAGR</th>
            <th>MoM change</th><th>MF / Stocks</th>
          </tr>
        </thead>
  ...
  // Re-populate kpis now that the element exists in DOM
  document.getElementById('snap-kpis').innerHTML = [
    {l:'Portfolio value',  v: fmtL(latest.totalValue),    s: momTag(valDelta),      a:'#d4a843'},
    {l:'Total invested',   v: fmtL(latest.totalInvested), s: momTag(invDelta),      a:'#58a6ff'},
    {l:'Total gain',       v: fmtL(latest.totalGain),     s: momTag(gainDelta),     a: latest.totalGain>=0?'#3fb950':'#f85149'},
...
function exportSnapshotsCSV() {
  ...
  const headers = ['Month','Total Value','Total Invested','Total Gain','Return %','MF CAGR %','MF Invested','Stock Invested','Funds','Stocks','Saved At'];
  const rows = snapshots.map(s => [
    s.label, Math.round(s.totalValue), Math.round(s.totalInvested), Math.round(s.totalGain),
    s.totalReturn?.toFixed(1)||'', s.mfCAGR?.toFixed(1)||'',
    Math.round(s.mfInvested||0), Math.round(s.stInvested||0),
    s.fundCount||'', s.stockCount||'', s.savedAt||''
  ]);
  const csv = [headers.join(','), ...rows.map(r=>r.join(','))].join('\n');
  const blob = new Blob([csv], {type:'text/csv'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = 'portfin_snapshot_history.csv';
