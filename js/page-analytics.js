...
const MF_CAT_SECTOR_WEIGHTS = {
  'Large Cap':       { Banking:0.30, IT:0.18, 'Energy/PSU':0.12, FMCG:0.10, 'Metals/Mining':0.08, 'Finance/PSU':0.08, Other:0.14 },
  'Large & Mid Cap': { Banking:0.22, IT:0.15, 'Energy/PSU':0.10, FMCG:0.08, 'Metals/Mining':0.08, 'Infra/PSU':0.10, Other:0.27 },
  'Mid Cap':         { Banking:0.15, IT:0.12, 'Infra/PSU':0.12, 'Metals/Mining':0.10, Defence:0.08, Renewables:0.07, Other:0.36 },
  'Small Cap':       { 'Infra/PSU':0.12, 'Metals/Mining':0.10, Defence:0.09, Renewables:0.08, Speculative:0.08, IT:0.08, Other:0.45 },
  'Flexi Cap':       { Banking:0.22, IT:0.16, FMCG:0.10, 'Energy/PSU':0.10, 'Metals/Mining':0.08, Other:0.34 },
  'Multi Cap':       { Banking:0.18, IT:0.14, FMCG:0.10, 'Energy/PSU':0.10, 'Metals/Mining':0.08, Other:0.40 },
  'ELSS':            { Banking:0.25, IT:0.16, FMCG:0.10, 'Energy/PSU':0.09, Other:0.40 },
  'Value':           { Banking:0.20, 'Energy/PSU':0.14, FMCG:0.12, 'Metals/Mining':0.10, 'Finance/PSU':0.10, Other:0.34 },
  'Index':           { Banking:0.33, IT:0.17, 'Energy/PSU':0.12, FMCG:0.09, 'Metals/Mining':0.07, Other:0.22 },
  'Other':           { Banking:0.20, IT:0.15, FMCG:0.10, Other:0.55 },
};
...
function renderOverlapDetector() {
  ...
  if (!totalPortInv) {
    el.innerHTML = '<div style="color:var(--muted);font-size:11px;padding:16px;text-align:center">Upload both files to detect portfolio overlap</div>';
    return;
  }
  ...
  const allSectors = [...new Set([...Object.keys(mfSectorExp), ...Object.keys(stSectorExp)])].filter(s => s !== 'Other');
  allSectors.sort((a, b) => {
  ...
  const overlapData = allSectors.map(sector => {
    ...
    return { sector, mfAmt, stAmt, totalAmt, totalPct, mfPct, stPct, hasOverlap, severity };
  ...
  const doubleExpPct   = totalPortInv > 0 ? totalDoubleExp / totalPortInv * 100 : 0;

  // ── 5. Verdict ─────────────────────────────────────────────
  let verdictColor, verdictIcon, verdictText;
  if (highOverlap.length > 0) {
    verdictColor = 'var(--red)'; verdictIcon = '⚠';
    ...
    verdictColor = 'var(--amber)'; verdictIcon = '◈';
    ...
    verdictColor = 'var(--blue)'; verdictIcon = '◎';
    ...
    verdictColor = 'var(--green)'; verdictIcon = '✓';
  ...
  const severityClr    = { high: 'var(--red)', medium: 'var(--amber)', low: 'var(--blue)', none: 'var(--muted)' };
  const severityBg     = { high: 'var(--red-bg)', medium: 'var(--amber-bg)', low: 'var(--blue-bg)', none: 'var(--bg4)' };
  const severityBorder = { high: 'var(--red-dim)', medium: '#4a3500', low: '#1a4060', none: 'var(--border)' };
  ...
  el.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:16px">
      ${[
        { l:'Double-exposed capital', v: fmtL(Math.round(totalDoubleExp)), s: doubleExpPct.toFixed(1)+'% of portfolio', c:'var(--amber)' },
        { l:'High overlap sectors',   v: highOverlap.length || '—',        s: highOverlap.length ? 'Needs attention' : 'None detected', c: highOverlap.length ? 'var(--red)' : 'var(--green)' },
        { l:'Moderate overlap',       v: medOverlap.length  || '—',        s: 'Sectors to monitor', c:'var(--amber)' },
        { l:'Well diversified',       v: noOverlap.length   || '—',        s: 'Single-path sectors', c:'var(--green)' },
      ].map(k => `<div class="tax-kpi"><div class="tax-kpi-label">${k.l}</div><div class="tax-kpi-val" style="color:${k.c}">${k.v}</div><div style="font-size:10px;color:var(--muted)">${k.s}</div></div>`).join('')}
    </div>
    ...
    <div style="margin-bottom:8px">
      ...
      ${overlapData.map(d => {
        ...
        const tip  = d.hasOverlap
          ...
          : (d.mfAmt > 0 ? 'Exposure only via MF funds' : 'Only in direct stocks');
    ...
    <div style="margin-top:4px;padding:12px 14px;background:var(--bg3);border:1px solid var(--border);border-radius:8px">
      ...
      ${[
        ...highOverlap.map(d => `<div style="display:flex;gap:8px;align-items:flex-start;padding:7px 0;border-bottom:1px solid var(--border);font-size:11px">
        ...
        ...medOverlap.map(d => `<div style="display:flex;gap:8px;align-items:flex-start;padding:7px 0;border-bottom:1px solid var(--border);font-size:11px">
...
function _buildOverlapBubble(data, maxBubble) {
  const W = 480, H = 260, padL = 54, padR = 16, padT = 16, padB = 48;
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    const gx = xScale(xv), gy = yScale(yv);
    ...
    if (i > 0) gridLines += `<text x="${padL-6}" y="${gy.toFixed(1)}" font-size="8" fill="var(--muted)" text-anchor="end" dominant-baseline="middle" font-family="DM Mono,monospace">${yv.toFixed(1)}%</text>`;
  }
  ...
  const bubbles = data.map(d => {
    ...
    const r   = Math.max(5, Math.min(22, Math.sqrt(d.totalAmt / maxBubble) * 28));
    const clr = OVERLAP_CLR[d.sector] || '#7d8590';
