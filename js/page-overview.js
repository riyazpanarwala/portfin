...
function renderOverview() {
  ...
  document.getElementById("kpi-overview").innerHTML = [
    ...
    },
    {
      l: "Total Gain",
      v: fmtL(k.totalGain),
      s: fmtP(k.totalReturn),
      sc: k.totalGain >= 0 ? "up" : "dn",
      a: "#3fb950",
    },
  ...
  document.getElementById("risk-alerts").innerHTML = alerts.length
    ? alerts
        .map(
          ([n, m]) =>
            `<div class="alert-row"><span class="alert-name">${n}</span><span class="alert-msg">${m}</span></div>`,
        )
        .join("")
...
function renderHealthScore() {
  ...
  const divScore = Math.max(
    0,
    20 -
      Math.max(0, (maxSingleStPct - 20) * 0.5) -
      Math.max(0, (specPct * 100 - 15) * 0.4),
  );
  ...
  const total = Math.round(
    divScore + mfScore + profScore + cagrScore + consistScore,
  );
  const clamp = Math.min(100, Math.max(0, total));
  const scoreColor =
    clamp >= 75 ? "#3fb950" : clamp >= 50 ? "#d4a843" : "#f85149";
  const scoreLabel =
    clamp >= 75 ? "Healthy" : clamp >= 50 ? "Fair" : "Needs Attention";
  ...
  const breakdown = [
    [
      ...
      Math.round(divScore),
      20,
      "Single-stock concentration & speculative exposure",
    ],
    ["MF Dominance", Math.round(mfScore), 20, "MF share of total portfolio"],
    [
      "Profitability",
      Math.round(profScore),
      20,
      "% of funds & stocks in profit",
    ],
    [
      "CAGR Quality",
      Math.round(cagrScore),
      20,
      "Average MF CAGR vs Nifty 50 (12%)",
    ],
    [
      "Consistency",
      Math.round(consistScore),
      20,
      "Regular investing across months",
    ],
  ];
...
function renderRiskReturnClassification() {
  ...
  const subtitleEl = document.getElementById("risk-return-subtitle");
  ...
  // 1. Small-cap / Speculative exposure
  ...
  if (topStockPct > 15)
    riskFactors.push(
      `⚠️ Top stock: ${topStockPct.toFixed(1)}% of equity portfolio`,
    );

  // 3. Volatility proxy (standard deviation of fund returns)
  const fundReturns = DATA.funds.map((f) => f.RetPct);
  const avgReturn =
    fundReturns.reduce((a, b) => a + b, 0) / (fundReturns.length || 1);
  const variance =
    fundReturns.reduce((a, b) => a + Math.pow(b - avgReturn, 2), 0) /
  ...
  if (volatilityProxy > 15)
    riskFactors.push(
      `📊 High return dispersion (${volatilityProxy.toFixed(1)}% std dev)`,
    );

  // 4. Negative CAGR holdings
  const negativeCAGR =
    DATA.funds.filter((f) => f.CAGR < 0).length +
    DATA.stocks.filter((s) => s.CAGR < 0).length;
  riskScore += Math.min(15, negativeCAGR * 3);
  if (negativeCAGR > 0)
    riskFactors.push(`📉 ${negativeCAGR} holding(s) with negative CAGR`);

  // Cap risk score at 100
  riskScore = Math.min(100, Math.max(0, riskScore));

  // Calculate Return Score (0-100, higher = better returns)
  let returnScore = 0;

  // Overall portfolio return vs benchmarks
  const portfolioReturn = k.totalReturn || 0;
  const niftyReturn = 12; // Approximate Nifty 10-year avg
  ...
  // MF CAGR quality
  const avgMFCAGR =
    DATA.funds.reduce((a, f) => a + f.CAGR, 0) / (DATA.funds.length || 1);
  ...
  // Cap return score at 100
  returnScore = Math.min(100, Math.max(0, returnScore));

  // Determine classification based on risk-return quadrant
  let classification = "";
  let color = "";
  let icon = "";
  let description = "";
  ...
  if (riskScore >= 60 && returnScore >= 60) {
    classification = "HIGH RISK · HIGH RETURN";
    color = "#d4a843";
    icon = "🚀";
    description =
      "Aggressive portfolio with strong returns. Your risk-taking is being rewarded.";
    recommendation =
      "Maintain this strategy but consider taking some profits off the table.";
  } else if (riskScore >= 60 && returnScore < 40) {
    classification = "HIGH RISK · LOW RETURN";
    color = "#f85149";
    icon = "⚠️";
    description =
      "You're taking significant risks but not seeing commensurate returns.";
    recommendation =
      "Review your speculative positions and consider shifting to quality large-caps or index funds.";
  } else if (riskScore >= 60 && returnScore >= 40 && returnScore < 60) {
    classification = "HIGH RISK · MODERATE RETURN";
    color = "#f0883e";
    icon = "📊";
    description = "You're taking above-average risk for average returns.";
    recommendation =
      "Consider reducing small-cap exposure and adding more diversified funds.";
  } else if (riskScore < 40 && returnScore >= 60) {
    classification = "LOW RISK · HIGH RETURN";
    color = "#3fb950";
    icon = "🏆";
    description =
      "Excellent risk-adjusted returns! You're beating the market without excessive risk.";
    recommendation =
      "This is the ideal zone — stay the course and keep SIPs running.";
  } else if (riskScore < 40 && returnScore < 40) {
    classification = "LOW RISK · LOW RETURN";
    color = "#7d8590";
    icon = "🐢";
    description =
      "Very conservative portfolio with returns below market average.";
    recommendation =
      "You may be too conservative. Consider adding some large-cap or index funds.";
  } else if (riskScore < 40 && returnScore >= 40 && returnScore < 60) {
    classification = "LOW RISK · MODERATE RETURN";
    color = "#58a6ff";
    icon = "⚖️";
    description = "Balanced portfolio with good risk management.";
    recommendation =
      "Well diversified. You could increase equity allocation slightly for better returns.";
  } else if (riskScore >= 40 && riskScore < 60 && returnScore >= 60) {
    classification = "MODERATE RISK · HIGH RETURN";
    color = "#a371f7";
    icon = "📈";
    description =
      "Great risk-return balance — you're getting premium returns for moderate risk.";
    recommendation =
      "This is excellent. Just monitor concentration in top performers.";
  } else {
    classification = "MODERATE RISK · MODERATE RETURN";
    color = "#79c0ff";
    icon = "◈";
    description = "A balanced approach with market-like returns.";
    recommendation =
      "Solid foundation. Consider adding one high-conviction fund to boost returns.";
  }

  // Get quadrant position for visual matrix
  const matrixX = returnScore; // X = Return (0-100)
  const matrixY = 100 - riskScore; // Y = Low risk at top (inverted)

  const riskColor =
    riskScore >= 60 ? "#f85149" : riskScore >= 40 ? "#f0883e" : "#3fb950";
  const returnColor =
    returnScore >= 60 ? "#3fb950" : returnScore >= 40 ? "#d4a843" : "#f85149";
    ...
    <div style="display:flex; flex-wrap:wrap; gap:20px; align-items:flex-start; justify-content:space-between;">
      <div style="flex:2; min-width:200px;">
        ...
        <div style="display:flex; gap:16px; margin:16px 0; flex-wrap:wrap;">
          <div class="risk-score-box" style="background:var(--bg4); border-radius:8px; padding:8px 14px; text-align:center; flex:1; min-width:80px;">
          ...
          <div class="return-score-box" style="background:var(--bg4); border-radius:8px; padding:8px 14px; text-align:center; flex:1; min-width:80px;">
        ...
        <div class="recommendation-box" style="background:${color}10; border-left:3px solid ${color}; border-radius:6px; padding:10px 12px; margin-top:8px;">
      ...
      <div style="flex:1; min-width:180px;">
        <div class="risk-return-matrix" style="position:relative; width:100%; max-width:280px; height:200px; background:var(--bg3); border-radius:12px; border:1px solid var(--border); margin:0 auto; overflow:hidden;">
          <!-- Quadrant labels -->
    ...
    <div style="margin-top:14px; padding-top:10px; border-top:1px solid var(--border);">
      ...
      <div style="display:flex; gap:8px; flex-wrap:wrap;">
        ${riskFactors
          ...
          .map(
            (f) =>
              `<span style="font-size:10px; background:var(--bg4); padding:3px 8px; border-radius:12px; color:var(--muted2);">${f}</span>`,
          )
          .join("")}
      </div>
    </div>
...
function renderSIPReminder() {
  ...
  const avgMonthly = activeMonths.length
    ? Math.round(
        activeMonths.reduce((a, x) => a + x.v, 0) / activeMonths.length,
      )
    : 0;
  ...
  const recommendedSIP =
    avgMonthly || Math.round((k.totalInvested || 0) * 0.02);
  const mfShare = k.totalInvested ? k.mfInvested / k.totalInvested : 0.7;
  const mfSIP = Math.round(
    recommendedSIP * Math.min(0.8, Math.max(0.5, mfShare)),
  );
...
function calculateDrawdown(series) {
  ...
  for (let i = 0; i < values.length; i++) {
    if (values[i] > runningPeak) {
      runningPeak = values[i];
      curPeakIdx = i;
    }
  ...
  const currentDD =
    allTimePeak > 0 ? (finalVal - allTimePeak) / allTimePeak : 0;
...
function calculateDrawdownWithPeriod(series) {
  if (!series.length)
    return { peakIndex: 0, troughIndex: 0, maxDD: 0, ddSeries: [] };
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const dd =
      runningPeak > 0 ? ((values[i] - runningPeak) / runningPeak) * 100 : 0;
